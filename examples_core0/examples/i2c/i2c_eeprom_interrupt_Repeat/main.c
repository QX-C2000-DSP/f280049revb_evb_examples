...
void I2C_init(uint32_t base, uint32_t speed)
{
    ...
    // Interrupt
    I2C_clearInterruptStatus(base, I2C_INT_ARB_LOST | I2C_INT_NO_ACK | I2C_INT_STOP_CONDITION | I2C_INT_REG_ACCESS_RDY);
...
uint16_t checkBusStatus(uint32_t base)
{
    ...
    {
        //DEBUG_LOG("Stop Condition Statu \r\n");
...
uint16_t I2C_ControllerReceiver(struct I2CHandle *I2C_Params)
{
    ...
    // 读取总值减1
    I2C_Params->NumOfDataBytes = I2C_Params->NumOfDataBytes - 1;
...
void handleI2C_ErrorCondition(struct I2CHandle *I2C_Params)
{
    ...
    // DEBUG_LOG("i2c_isr: intSource=0x%x \r\n", intSource);
        ...
        case I2C_INTSRC_REG_ACCESS_RDY: // 3
            ...
            if (interruptEnabled) {
                ...
                if (I2C_Params->numofFifoLevel) {
                    I2C_setFIFOInterruptLevel(base, I2C_FIFO_TXEMPTY, I2C_FIFO_LEVEL);
                } else {
...
void Write_Read_TX_RX_FIFO(struct I2CHandle *I2C_Params)
{
    ...
    {
        // DEBUG_LOG("I2C_INT_RXFF: fifo_num =%d, remain=%d, HasRecv=%d \r\n", \
            I2C_Params->numofFifoLevel, I2C_Params->remainingBytes, I2C_Params->numofHasRecvBytes);

        I2C_disableInterrupt(base, I2C_INT_RXFF);

        // 先按预设的 I2C_FIFO_LEVEL 深度读取数据
        if (I2C_Params->numofFifoLevel) {

            for (i = 0; i < I2C_FIFO_LEVEL; i++)
            {
                *(currentPtr->pRX_MsgBuffer) = I2C_getData(base);
                currentPtr->pRX_MsgBuffer++;
            }

            I2C_Params->numofHasRecvBytes += I2C_FIFO_LEVEL;
        ...
        } else { // remaining 剩余数据部分

            //DEBUG_LOG("recv remaining: %d \r\n", I2C_Params->remainingBytes);

            for (i = 0; i < I2C_Params->remainingBytes; i++) {
                *(currentPtr->pRX_MsgBuffer++) = I2C_getData(base);
            }

            I2C_Params->numofHasRecvBytes += I2C_Params->remainingBytes;
        }

        // 检查是否还有剩余数据未读取接收, 等于0则表示已经接收完成
        if (I2C_Params->numofHasRecvBytes == I2C_Params->NumOfDataBytes) {
            ...
            if (HWREGH(EEPROM.base + I2C_O_MDR) & I2C_MDR_MST) {
                ...
                I2C_sendStopCondition(base);
                //DEBUG_LOG(" send Rx STOP \r\n");

                // 之前读取总值减1, 现在回来 补读一次
                timeout = 0x100 * 10;
                while((1 != I2caRegs.I2CFFRX.bit.RXFFST) && timeout--);
...
int main(void)
{
    ...
    DEBUG_LOG("I2C Interrupt Repeat(FIFO) Test \r\n");
