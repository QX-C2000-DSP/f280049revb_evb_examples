...
void DCSM_unlockZone1CSM(const DCSM_CSMPasswordKey *const psCMDKey)
{
    ...
    uint32_t zsbBase  = (DCSMBANK0_Z1OTP_BASE + 0x20U); // base address of the ZSB
    int32_t bitPos    = 28;
    int32_t zeroFound = 0;
...
DCSM_EXEOnlyStatus DCSM_getZone1FlashEXEStatus(DCSM_Sector sector)
{
    ...
    else
    {
        ...
        //
        status = (DCSM_EXEOnlyStatus)((uint16_t)((regValue >> statusBitShift) & 0x01U));
    }
    return (status);
}
...
DCSM_EXEOnlyStatus DCSM_getZone1RAMEXEStatus(DCSM_RAMModule module)
{
    ...
    else
    {
        ...
        //
        status = (DCSM_EXEOnlyStatus)(uint16_t)((HWREGH(DCSMBANK0_Z1_BASE + DCSM_O_Z1_EXEONLYRAMR)
                                                    >> (uint16_t)module)
                                                & 0x01U);
    }
    return (status);
...
DCSM_EXEOnlyStatus DCSM_getZone2RAMEXEStatus(DCSM_RAMModule module)
{
    ...
    else
    {
        ...
        // Get the EXE status of the RAM Module
        //
        status = (DCSM_EXEOnlyStatus)(uint16_t)((HWREGH(DCSMBANK0_Z2_BASE + DCSM_O_Z2_EXEONLYRAMR)
                                                    >> (uint16_t)module)
                                                & 0x01U);
    }
    return (status);
...
bool DCSM_claimZoneSemaphore(DCSM_SemaphoreZone zone)
{
    ...
    //
    HWREGH(regAddress) = ((uint16_t)FLSEM_KEY << DCSM_FLSEM_KEY_S) | (uint16_t)zone;
    EDIS;
    ...
    // return false
    //
    return (((HWREGH(regAddress) & DCSM_FLSEM_SEM_M) == (uint16_t)zone) ? true : false);
}
...
//
//*****************************************************************************
void DCSM_setConfig(uint32_t config, uint32_t data)
{
    HWREG(DCSMCOMMON_BASE + DCSM_USER_CONFIG) = config;
    HWREG(DCSMCOMMON_BASE + DCSM_USER_DATA)   = data;
}
