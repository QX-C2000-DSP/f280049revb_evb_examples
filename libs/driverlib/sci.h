...
//
//*****************************************************************************
static inline void SCI_getFIFOInterruptLevel(
    uint32_t base, SCI_TxFIFOLevel *txLevel, SCI_RxFIFOLevel *rxLevel)
{
...
//
//*****************************************************************************
static inline void SCI_getConfig(uint32_t base, uint32_t lspclkHz, uint32_t *baud, uint32_t *config)
{
    //
    // Check the arguments.
    //
    ...
    //
    *baud = lspclkHz
            / ((1U + (((uint32_t)HWREGH(base + SCI_O_HBAUD) << 8U) | HWREGH(base + SCI_O_LBAUD)))
                * 8U);
...
static inline bool SCI_isDataAvailableNonFIFO(uint32_t base)
{
    ...
    //
    return (((HWREGH(base + SCI_O_RXST) & SCI_RXST_RXRDY) == SCI_RXST_RXRDY) ? true : false);
}
...
static inline SCI_TxFIFOLevel SCI_getTxFIFOStatus(uint32_t base)
{
    ...
    //
    return (
        (SCI_TxFIFOLevel)((HWREGH(base + SCI_O_FFTX) & SCI_FFTX_TXFFST_M) >> SCI_FFTX_TXFFST_S));
}
...
static inline SCI_RxFIFOLevel SCI_getRxFIFOStatus(uint32_t base)
{
    ...
    //
    return (
        (SCI_RxFIFOLevel)((HWREGH(base + SCI_O_FFRX) & SCI_FFRX_RXFFST_M) >> SCI_FFRX_RXFFST_S));
}
...
static inline bool SCI_isTransmitterBusy(uint32_t base)
{
    ...
    if (SCI_isFIFOEnabled(base))
    {
        ...
        //
        return (((HWREGH(base + SCI_O_FFTX) & SCI_FFTX_TXFFST_M) != 0U) ? true : false);
    }
    else
    {
        //
        ...
        //
        return (
            ((HWREGH(base + SCI_O_CTL2) & SCI_CTL2_TXEMPTY) == SCI_CTL2_TXEMPTY) ? false : true);
    }
}
...
//
//*****************************************************************************
extern void SCI_writeCharArray(uint32_t base, const uint8_t *const array, uint16_t length);
...
//
//*****************************************************************************
extern void SCI_readCharArray(uint32_t base, uint8_t *const array, uint16_t length);
