...
GPIO_Direction GPIO_getDirectionMode(uint32_t pin)
{
    ...
    return ((GPIO_Direction)((uint32_t)((gpioBaseAddr[GPIO_GPxDIR_INDEX] >> (pin % 32U)) & 1U)));
}
...
GPIO_QualificationMode GPIO_getQualificationMode(uint32_t pin)
{
    ...
    //
    qualRes = (gpioBaseAddr[qSelIndex] >> shiftAmt) & (uint32_t)GPIO_GPAQSEL1_GPIO0_M;
...
{
    // no need, just for compatibility
}
...
void GPIO_setPinConfig(uint32_t pinConfig)
{
    ...
    //
    HWREG(muxRegAddr + GPIO_MUX_TO_GMUX) = (HWREG(muxRegAddr + GPIO_MUX_TO_GMUX) & ~pinMask)
...
//*****************************************************************************
void GPIO_MuxConfig(uint32_t pin, uint32_t pinConfig, uint32_t pinType, GPIO_QualificationMode qual)
{
