...
typedef enum
{
    ...
    // XBARFLG4
    //
    XBAR_INPUT_FLG_ERRORSTS = 0x031C
} XBAR_InputFlag;
#endif
...
static inline void XBAR_setOutputLatchMode(XBAR_OutputNum output, bool enable)
{
    ...
    if (enable)
    {
        HWREGH(OUTPUTXBAR_BASE + XBAR_O_OUTPUTLATCHENABLE) |= 0x1U << ((uint16_t)output / 4U);
    }
    else
    {
        HWREGH(OUTPUTXBAR_BASE + XBAR_O_OUTPUTLATCHENABLE) &= ~(0x1U << ((uint16_t)output / 4U));
...
static inline bool XBAR_getOutputLatchStatus(XBAR_OutputNum output)
{
    ...
    //
    return (
        (HWREGH(OUTPUTXBAR_BASE + XBAR_O_OUTPUTLATCH) & (0x1U << ((uint16_t)output / 4U))) != 0U);
}
...
static inline void XBAR_clearOutputLatch(XBAR_OutputNum output)
{
    ...
    //
    HWREGH(OUTPUTXBAR_BASE + XBAR_O_OUTPUTLATCHCLR) |= 0x1U << ((uint16_t)output / 4U);
}
...
static inline void XBAR_forceOutputLatch(XBAR_OutputNum output)
{
    ...
    //
    HWREGH(OUTPUTXBAR_BASE + XBAR_O_OUTPUTLATCHFRC) = (uint16_t)0x1U << ((uint16_t)output / 4U);
}
...
static inline void XBAR_invertOutputSignal(XBAR_OutputNum output, bool invert)
{
    ...
    if (invert)
    {
        HWREGH(OUTPUTXBAR_BASE + XBAR_O_OUTPUTINV) |= 0x1U << ((uint16_t)output / 4U);
    }
    else
    {
        HWREGH(OUTPUTXBAR_BASE + XBAR_O_OUTPUTINV) &= ~(0x1U << ((uint16_t)output / 4U));
...
static inline void XBAR_invertEPWMSignal(XBAR_TripNum trip, bool invert)
{
    ...
    if (invert)
    {
        HWREGH(EPWMXBAR_BASE + XBAR_O_TRIPOUTINV) |= 0x1U << ((uint16_t)trip / 4U);
    }
    else
    {
        HWREGH(EPWMXBAR_BASE + XBAR_O_TRIPOUTINV) &= ~(0x1U << ((uint16_t)trip / 4U));
...
static inline void XBAR_setInputPin(XBAR_InputNum input, uint16_t pin)
{
    ...
    HWREGH(XBAR_INPUT_BASE + (uint16_t)input * 4) = pin;
