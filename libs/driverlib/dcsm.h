...
#define DCSM_O_Z1_CSMPSWD3 0x2cU //!< Z1 CSMPSWD3 offset
...
static inline DCSM_SecurityStatus DCSM_getZone1CSMSecurityStatus(void)
{
    ...
    // Else it is secure.
    //
    if (((status & DCSM_Z1_CR_ARMED) != 0U) && ((status & DCSM_Z1_CR_UNSECURE) != 0U))
    {
...
static inline DCSM_SecurityStatus DCSM_getZone2CSMSecurityStatus(void)
{
    ...
    // Else it is secure.
    //
    if (((status & DCSM_Z2_CR_ARMED) != 0U) && ((status & DCSM_Z2_CR_UNSECURE) != 0U))
    {
        returnStatus = DCSM_STATUS_UNSECURE;
    }
...
static inline DCSM_MemoryStatus DCSM_getRAMZone(DCSM_RAMModule module)
{
    ...
    //
    return (
        (DCSM_MemoryStatus)(uint32_t)((HWREG(DCSMCOMMON_BASE + DCSM_O_RAMSTAT) >> shift) & 0x03U));
}
...
//
//*****************************************************************************
static inline uint32_t DCSM_getZone1LinkPointerError(DCSM_Bank bank)
{
    uint32_t tempReturn;
...
//
//*****************************************************************************
static inline uint32_t DCSM_getZone2LinkPointerError(DCSM_Bank bank)
{
    uint32_t tempReturn;
...
//
//*****************************************************************************
extern void DCSM_setConfig(uint32_t config, uint32_t data);
