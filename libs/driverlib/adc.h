...
#define ADC_INT_REF_TSSLOPE  0.006F
#define ADC_INT_REF_TSOFFSET 1.1F
...
#define ADC_SAFECHECK_STATUS_MASK \
    (ADC_SAFECHECK_RESULT1_READY | ADC_SAFECHECK_RESULT2_READY | ADC_SAFECHECK_RESULT_OOT)
...
typedef enum
{
    ADC_CLK_DIV_1_0  = 1U,  //!< ADCCLK = (input clock) / 2.0
...
typedef enum
{
    ...
    ADC_TRIGGER_CPU2_TINT0  = 21U, //!< CPU2 Timer 0, TINT0
    ADC_TRIGGER_CPU2_TINT1  = 22U, //!< CPU2 Timer 1, TINT1
    ADC_TRIGGER_CPU2_TINT2  = 23U, //!< CPU2 Timer 2, TINT2
    ADC_TRIGGER_EPWM9_SOCA  = 24U, //!< ePWM9, ADCSOCA
    ADC_TRIGGER_EPWM9_SOCB  = 25U, //!< ePWM9, ADCSOCB
    ADC_TRIGGER_EPWM10_SOCA = 26U, //!< ePWM10, ADCSOCA
    ADC_TRIGGER_EPWM10_SOCB = 27U, //!< ePWM10, ADCSOCB
    ADC_TRIGGER_EPWM11_SOCA = 28U, //!< ePWM11, ADCSOCA
    ADC_TRIGGER_EPWM11_SOCB = 29U, //!< ePWM11, ADCSOCB
    ADC_TRIGGER_EPWM12_SOCA = 30U, //!< ePWM12, ADCSOCA
    ADC_TRIGGER_EPWM12_SOCB = 31U, //!< ePWM12, ADCSOCB
    ADC_TRIGGER_INT1        = 32U, //!< ADC INT0
    ADC_TRIGGER_INT2        = 33U, //!< ADC INT1
} ADC_Trigger;
...
{
    return ((base == ADCA_BASE) || (base == ADCB_BASE) || (base == ADCC_BASE) || (base == ADCG_BASE)
            || (base == ADCH_BASE));
}
#endif
...
static inline void ADC_setPrescaler(uint32_t base, ADC_ClkPrescale clkPrescale)
{
    ...
    //
    EALLOW;
    HWREG(base + ADC_O_CTL2)
        = (HWREG(base + ADC_O_CTL2) & ~ADC_CTL2_PRESCALE_M) | (uint32_t)clkPrescale;
    EDIS;
}
    ...
    ADC_Channel channel, uint32_t sampleWindow)
{
    ...
    HWREG(ctlRegAddr) = (HWREG(ctlRegAddr) & ~(mask)) | ((uint32_t)channel << ADC_SOC0CTL_CHSEL_S)
                        | ((uint32_t)trigger << ADC_SOC0CTL_TRIGSEL_S) | sampleWindow;
    ...
    uint32_t base, ADC_SOCNumber socNumber, ADC_IntSOCTrigger trigger)
{
    ...
    if ((uint32_t)socNumber <= 7)
    {
        ...
        intRegAddr = base + ADC_O_INTSOCSEL1;
    }
    else
    {
        shiftVal   = ((uint32_t)socNumber - 8U) << 1U;
        intRegAddr = base + ADC_O_INTSOCSEL1 + 4U;
    }
    //
    ...
    //
    EALLOW;
    HWREG(intRegAddr) = (HWREG(intRegAddr) & ~((uint32_t)ADC_INTSOCSEL1_SOC0_M << shiftVal))
...
static inline void ADC_setInterruptPulseMode(uint32_t base, ADC_PulseMode pulseMode)
{
    ...
    //
    EALLOW;
    HWREG(base + ADC_O_CTL1)
        = (HWREG(base + ADC_O_CTL1) & ~ADC_CTL1_INTPULSEPOS) | (uint32_t)pulseMode;
    EDIS;
}
...
//
//*****************************************************************************
static inline void ADC_setInterruptCycleOffset(uint32_t base, uint32_t cycleOffset)
{
    //
    // Check the arguments.
    //
...
static inline void ADC_forceSOC(uint32_t base, ADC_SOCNumber socNumber)
{
    ...
    //
    HWREG(base + ADC_O_SOCFRC1) = ((uint32_t)1U << (uint32_t)socNumber);
}
...
static inline void ADC_clearInterruptStatus(uint32_t base, ADC_IntNumber adcIntNum)
{
    ...
    //
    HWREG(base + ADC_O_INTFLGCLR) = (uint32_t)1U << (uint32_t)adcIntNum;
}
...
static inline void ADC_clearInterruptOverflowStatus(uint32_t base, ADC_IntNumber adcIntNum)
{
    ...
    //
    HWREG(base + ADC_O_INTOVFCLR) = (uint32_t)1U << (uint32_t)adcIntNum;
}
...
//
//*****************************************************************************
static inline uint32_t ADC_readResult(uint32_t resultBase, ADC_SOCNumber socNumber)
{
    //
    // Check the arguments.
    //
    ...
    //
    return (HWREG(resultBase + (uint32_t)ADC_RESULTx_OFFSET_BASE + (uint32_t)socNumber * 4U));
}
...
//
//*****************************************************************************
static inline void ADC_setBurstModeConfig(uint32_t base, ADC_Trigger trigger, uint32_t burstSize)
{
    uint32_t regValue;
    ...
    regValue = (uint32_t)trigger | ((burstSize - 1U) << ADC_BURSTCTL_BURSTSIZE_S);

    HWREG(base + ADC_O_BURSTCTL)
        = (HWREG(base + ADC_O_BURSTCTL)
              & ~((uint32_t)ADC_BURSTCTL_BURSTTRIGSEL_M | ADC_BURSTCTL_BURSTSIZE_M))
          | regValue;
...
static inline void ADC_setSOCPriority(uint32_t base, ADC_PriorityMode priMode)
{
    ...
    HWREG(base + ADC_O_SOCPRICTL)
        = (HWREG(base + ADC_O_SOCPRICTL) & ~ADC_SOCPRICTL_SOCPRIORITY_M) | (uint32_t)priMode;
...
static inline void ADC_setupPPB(uint32_t base, ADC_PPBNumber ppbNumber, ADC_SOCNumber socNumber)
{
    ...
    //
    EALLOW;
    HWREG(base + ppbOffset) = (HWREG(base + ppbOffset) & ~ADC_PPB1CONFIG_CONFIG_M)
...
//
//*****************************************************************************
static inline void ADC_enablePPBEvent(uint32_t base, ADC_PPBNumber ppbNumber, uint32_t evtFlags)
{
    //
    // Check the arguments.
    //
...
//
//*****************************************************************************
static inline void ADC_disablePPBEvent(uint32_t base, ADC_PPBNumber ppbNumber, uint32_t evtFlags)
{
    //
    // Check the arguments.
    //
...
static inline void ADC_enablePPBEventInterrupt(
    uint32_t base, ADC_PPBNumber ppbNumber, uint32_t intFlags)
{
    //
    // Check the arguments.
    //
...
//
//*****************************************************************************
static inline uint32_t ADC_getPPBEventStatus(uint32_t base, ADC_PPBNumber ppbNumber)
{
    //
    // Check the arguments.
    //
    ...
    //
    return ((HWREG(base + ADC_O_EVTSTAT) >> ((uint32_t)ppbNumber * 4U)) & 0x7U);
}
...
static inline void ADC_clearPPBEventStatus(
    uint32_t base, ADC_PPBNumber ppbNumber, uint32_t evtFlags)
{
    //
    // Check the arguments.
    //
...
static inline int32_t ADC_readPPBResult(uint32_t resultBase, ADC_PPBNumber ppbNumber)
{
    ...
    return ((int32_t)HWREG(
        resultBase + (uint32_t)ADC_PPBxRESULT_OFFSET_BASE + ((uint32_t)ppbNumber * 4UL)));
}
...
//
//*****************************************************************************
static inline uint32_t ADC_getPPBDelayTimeStamp(uint32_t base, ADC_PPBNumber ppbNumber)
{
    uint32_t ppbOffset;
    ...
    uint32_t base, ADC_PPBNumber ppbNumber, int16_t offset)
{
    ...
    //
    EALLOW;
    HWREG(base + ppbOffset) = (HWREG(base + ppbOffset) & ~ADC_PPB1OFFCAL_OFFCAL_M)
...
static inline void ADC_setPPBReferenceOffset(
    uint32_t base, ADC_PPBNumber ppbNumber, uint32_t offset)
{
    uint32_t ppbOffset;
...
static inline void ADC_enableInterrupt(uint32_t base, ADC_IntNumber adcIntNum)
{
    ...
    //
    intRegAddr = base + ADC_INTSELxNy_OFFSET_BASE + (((uint32_t)adcIntNum >> 1) * 4U);
...
static inline void ADC_setInterruptSource(
    uint32_t base, ADC_IntNumber adcIntNum, uint32_t intTrigger)
{
    uint32_t intRegAddr;
    uint32_t shiftVal;
    ...
    HWREG(intRegAddr) = (HWREG(intRegAddr) & ~(ADC_INTSEL1N2_INT1SEL_M << shiftVal))
...
//
//*****************************************************************************
static inline int16_t ADC_getTemperatureC(
    uint32_t tempResult, ADC_ReferenceMode refMode, float32_t vref)
{
    float32_t temp;
    ...
    //
    temp = (float32_t)tempResult / 4095 * vref;
    if (refMode == ADC_REFERENCE_INTERNAL)
    {
        return ((float32_t)((((float32_t)temp - ADC_INT_REF_TSOFFSET)) / ADC_INT_REF_TSSLOPE));
    }
    else
    {
        return ((float32_t)((((float32_t)temp - ADC_EXT_REF_TSOFFSET)) / ADC_EXT_REF_TSSLOPE));
    }
}
...
//
//*****************************************************************************
static inline int16_t ADC_getTemperatureK(
    uint32_t tempResult, ADC_ReferenceMode refMode, float32_t vref)
{
    float32_t temp;
    ...
    {
        return (
            (float32_t)((((float32_t)temp - ADC_INT_REF_TSOFFSET)) / ADC_INT_REF_TSSLOPE) + 273);
    }
    else
    {
        return (
            (float32_t)((((float32_t)temp - ADC_EXT_REF_TSOFFSET)) / ADC_EXT_REF_TSSLOPE) + 273);
    }
}
