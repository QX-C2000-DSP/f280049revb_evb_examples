...
{
    return ((pin <= 120U) || ((pin >= 224U) && (pin <= 247U)));
}
#endif
...
static inline void GPIO_setInterruptType(GPIO_ExternalIntNum extIntNum, GPIO_IntType intType)
{
    ...
    //
    HWREG(XINT_BASE + ((uint16_t)extIntNum * 4))
        = (HWREG(XINT_BASE + ((uint16_t)extIntNum * 4)) & ~XINT_1CR_POLARITY_M) | (uint16_t)intType;
}
...
static inline GPIO_IntType GPIO_getInterruptType(GPIO_ExternalIntNum extIntNum)
{
    ...
    //
    return ((GPIO_IntType)(HWREGH(XINT_BASE + ((uint16_t)extIntNum * 4)) & XINT_1CR_POLARITY_M));
}
...
static inline void GPIO_enableInterrupt(GPIO_ExternalIntNum extIntNum)
{
    ...
    //
    HWREGH(XINT_BASE + ((uint16_t)extIntNum * 4)) |= XINT_1CR_ENABLE;
}
...
static inline uint16_t GPIO_getInterruptCounter(GPIO_ExternalIntNum extIntNum)
{
    ...
    //
    return ((HWREGH(XINT_BASE + XINT_O_1CTR + (uint16_t)extIntNum * 4)));
}
...
static inline uint32_t GPIO_readPortData(GPIO_Port port)
{
    ...
    //
    gpioDataReg = (uint32_t *)((uintptr_t)GPIODATA_BASE) + ((uint32_t)port * GPIO_DATA_REGS_STEP);
...
static inline void GPIO_lockPortConfig(GPIO_Port port, uint32_t pinMask)
{
    ...
    //
    gpioDataReg = (uint32_t *)((uintptr_t)GPIOCTRL_BASE) + ((uint32_t)port * GPIO_CTRL_REGS_STEP);
...
//*****************************************************************************
extern void GPIO_MuxConfig(
    uint32_t pin, uint32_t pinConfig, uint32_t pinType, GPIO_QualificationMode qual);
