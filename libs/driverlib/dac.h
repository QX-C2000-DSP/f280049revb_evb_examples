...
typedef enum
{
    ...
    DAC_LOAD_PWMSYNC = 8  //!< Load on next PWMSYNC specified by SYNCSEL
} DAC_LoadMode;

//*****************************************************************************
//
//! Values that can be passed to DAC_setPWMSyncSignal() as the \e mode parameter.
//
//*****************************************************************************
typedef enum
{
...
static inline void DAC_setReferenceVoltage(uint32_t base, DAC_ReferenceVoltage source)
{
    ...
    HWREGH(base + DAC_O_CTL) = (HWREGH(base + DAC_O_CTL) & ~DAC_CTL_DACREFSEL_M) | (uint16_t)source;
...
static inline void DAC_setGainMode(uint32_t base, DAC_GainMode mode)
{
    ...
    HWREGH(base + DAC_O_CTL)
        = ((HWREGH(base + DAC_O_CTL) & ~DAC_CTL_MODE_M)) | (mode << DAC_CTL_MODE_S);
...
static inline void DAC_setLoadMode(uint32_t base, DAC_LoadMode mode)
{
    ...
    HWREGH(base + DAC_O_CTL) = (HWREGH(base + DAC_O_CTL) & ~DAC_CTL_LOADMODE) | (uint16_t)mode;
...
static inline void DAC_setShadowValue(uint32_t base, uint16_t value)
{
    ...
    //
    HWREGH(base + DAC_O_VALS) = (HWREGH(base + DAC_O_VALS) & ~DAC_VALS_DACVALS_M)
...
static inline int16_t DAC_getOffsetTrim(uint32_t base)
{
    ...
    value = ((value & (uint16_t)DAC_REG_BYTE_MASK) ^ (uint16_t)0x80) - (uint16_t)0x80;
