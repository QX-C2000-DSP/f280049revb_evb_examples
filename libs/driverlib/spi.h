...
static inline void SPI_setcharLength(uint32_t base, uint16_t charLength)
{
    ...
    HWREGH(base + SPI_O_CCR) = (HWREGH(base + SPI_O_CCR) & ~SPI_CCR_SPICHAR_M) | (charLength - 1U);
    ...
    uint32_t base, SPI_TxFIFOLevel *txLevel, SPI_RxFIFOLevel *rxLevel)
{
    ...
    //
    *txLevel = (SPI_TxFIFOLevel)((uint16_t)(HWREGH(base + SPI_O_FFTX) & SPI_FFTX_TXFFIL_M));
    *rxLevel = (SPI_RxFIFOLevel)((uint16_t)(HWREGH(base + SPI_O_FFRX) & SPI_FFRX_RXFFIL_M));
}
...
static inline SPI_TxFIFOLevel SPI_getTxFIFOStatus(uint32_t base)
{
    ...
    return ((SPI_TxFIFOLevel)((
        uint16_t)((HWREGH(base + SPI_O_FFTX) & SPI_FFTX_TXFFST_M) >> SPI_FFTX_TXFFST_S)));
}
...
static inline SPI_RxFIFOLevel SPI_getRxFIFOStatus(uint32_t base)
{
    ...
    return ((SPI_RxFIFOLevel)((
        uint16_t)((HWREGH(base + SPI_O_FFRX) & SPI_FFRX_RXFFST_M) >> SPI_FFRX_RXFFST_S)));
}
...
static inline void SPI_setPTESignalPolarity(uint32_t base, SPI_PTEPolarity polarity)
{
    ...
    //
    HWREGH(base + SPI_O_PRI) = (HWREGH(base + SPI_O_PRI) & ~SPI_PRI_PTEINV) | (uint16_t)polarity;
}
...
static inline void SPI_setEmulationMode(uint32_t base, SPI_EmulationMode mode)
{
    ...
    HWREGH(base + SPI_O_PRI)
        = (HWREGH(base + SPI_O_PRI) & ~(SPI_PRI_FREE | SPI_PRI_SOFT)) | (uint16_t)mode;
}
