...
typedef enum
{
    ...
    PGA_GAIN_48 = 0x80  //!< PGA gain value of 48
} PGA_GainValue;
...
typedef enum
{
    ...
    PGA_LOW_PASS_FILTER_RESISTOR_200_OHM = 1,
    //! Resistor value of 160 Ohm
    PGA_LOW_PASS_FILTER_RESISTOR_160_OHM = 5,
    //! Resistor value of 130 Ohm, (WAINNING:accurate resistance value is 6.15)
    ...
    PGA_LOW_PASS_FILTER_RESISTOR_50_OHM = 16
...
//! PGA Register GAIN48TRIM
//!
#define PGA_REGISTER_GAIN48TRIM PGA_LOCK_PGAGAIN48TRIM
...
static inline void PGA_setGain(uint32_t base, PGA_GainValue gainValue)
{
    ...
    HWREG(base + PGA_O_CTL)
        = ((HWREG(base + PGA_O_CTL) & ~((uint32_t)PGA_CTL_GAIN_M)) | (uint16_t)gainValue);
    EDIS;
}
...
static inline void PGA_setFilterResistor(uint32_t base, PGA_LowPassResistorValue resistorValue)
{
    ...
    if (resistorValue)
    {
        HWREG(base + PGA_O_CTL) |= ((uint32_t)PGA_CTL_FILTREEN);
        HWREG(base + PGA_O_CTL) = ((HWREG(base + PGA_O_CTL) & ~((uint32_t)PGA_CTL_FILTRESSEL_M))
