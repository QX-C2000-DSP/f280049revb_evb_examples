...
static inline bool CPUTimer_getTimerOverflowStatus(uint32_t base)
{
    ...
    //
    return (
        ((HWREGH(base + CPUTIMER_O_TCR) & CPUTIMER_TCR_TIF) == CPUTIMER_TCR_TIF) ? true : false);
}
    ...
    uint32_t base, CPUTimer_ClockSource source, CPUTimer_Prescaler prescaler)
{
    ...
    if (base == CPUTIMER2_BASE)
    {
        ...
        HWREGH(CPUSYS_BASE + SYSCTL_O_TMR2CLKCTL)
            = (HWREGH(CPUSYS_BASE + SYSCTL_O_TMR2CLKCTL) & ~SYSCTL_TMR2CLKCTL_TMR2CLKSRCSEL_M)
              | (uint16_t)source;
        ...
        //
        HWREGH(CPUSYS_BASE + SYSCTL_O_TMR2CLKCTL)
            = (HWREGH(CPUSYS_BASE + SYSCTL_O_TMR2CLKCTL) & ~SYSCTL_TMR2CLKCTL_TMR2CLKPRESCALE_M)
        ...
        //
        HWREGH(CPUSYS_BASE + SYSCTL_O_TMR2CLKCTL)
            = (HWREGH(CPUSYS_BASE + SYSCTL_O_TMR2CLKCTL) & ~SYSCTL_TMR2CLKCTL_CPU2TMR2CLKSRCSEL_M)
              | (uint16_t)source;
        SYSCTL_REGWRITE_DELAY;
        ...
        //
        HWREGH(CPUSYS_BASE + SYSCTL_O_TMR2CLKCTL)
            = (HWREGH(CPUSYS_BASE + SYSCTL_O_TMR2CLKCTL) & ~SYSCTL_TMR2CLKCTL_CPU2TMR2CLKPRESCALE_M)
              | ((uint16_t)prescaler << SYSCTL_TMR2CLKCTL_CPU2TMR2CLKPRESCALE_S);
#endif
        EDIS;
    }
    SYSCTL_REGWRITE_DELAY;
...
//
//*****************************************************************************
// extern void CPUTimer_setEmulationMode(uint32_t base, CPUTimer_EmulationMode mode);
