    ...
    uint32_t bitRate, uint16_t dataWidth)
{
    ...
    //
    regValue = (((uint16_t)protocol << 6U) & SPI_CCR_CLKPOLARITY) | (dataWidth - 1U);

    HWREGH(base + SPI_O_CCR)
        = (HWREGH(base + SPI_O_CCR) & ~(SPI_CCR_CLKPOLARITY | SPI_CCR_SPICHAR_M)) | regValue;
    ...
    uint32_t base, SPI_endianess endianness, uint16_t dummyData, uint16_t txDelay)
{
    ...
    if (endianness == SPI_DATA_LITTLE_ENDIAN)
    {
        ...
        //
        rxData
            = ((uint32_t)rxBuffer[2] << 16) | ((uint32_t)rxBuffer[1] << 8) | (uint32_t)rxBuffer[0];
    }
    else
    {
        //
        ...
        //
        rxData
            = ((uint32_t)rxBuffer[0] << 16) | ((uint32_t)rxBuffer[1] << 8) | (uint32_t)rxBuffer[2];
    }
