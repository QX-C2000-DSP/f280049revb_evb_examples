...
static inline bool EPWM_isBaseValid(uint32_t base)
{
    return ((base == EPWM1_BASE) || (base == EPWM2_BASE) || (base == EPWM3_BASE)
           ...
           || (base == EPWM9_BASE || (base == EPWM10_BASE) || (base == EPWM11_BASE)
               || (base == EPWM12_BASE));
}
#endif
...
//
//*****************************************************************************
static inline void EPWM_setSyncInPulseSource(uint32_t base, EPWM_SyncInPulseSource source)
{
    //
    // Check the arguments
    //
    ...
    //
    HWREGH(base + EPWM_O_EPWMSYNCINSEL) = (HWREGH(base + EPWM_O_EPWMSYNCINSEL) & (~EPWM_EPWMSYNCINSEL_SEL_M))
                                      | ((uint16_t)source & EPWM_EPWMSYNCINSEL_SEL_M);
}
    ...
    uint32_t base, EPWM_CounterCompareModule compModule)
{
    ...
    if ((compModule == EPWM_COUNTER_COMPARE_A) || (compModule == EPWM_COUNTER_COMPARE_B))
    {
        ...
        //
        compCount = (uint16_t)((HWREG(base + registerOffset) & 0xFFFF0000UL) >> 16U);
    }
    else
    {
        //
...
static inline void EPWM_configureDigitalCompareCounterCaptureMode(
    uint32_t base, bool disableClearMode)
{
    //
    // Check the arguments
    //
    ...
    uint32_t base, uint16_t tripInput, EPWM_DigitalCompareType dcType)
{
    ...
    //
    registerOffset = EPWM_O_DCAHTRIPSEL + (uint32_t)dcType * 2;
...
static inline void EPWM_lockRegisters(uint32_t base, EPWM_LockRegisterGroup registerGroup)
{
    ...
    //
    HWREG(base + EPWM_O_LOCK) = (0xA5A50000UL | ((uint32_t)registerGroup));
}
